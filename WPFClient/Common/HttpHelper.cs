...
namespace WPFClient.Util
{
    ...
    public class HttpHelper
    {
       ...
       static log4net.ILog log = log4net.LogManager.GetLogger(typeof(HttpHelper));

        public static string QINIU_CDN_URL = "http://stcdn.91asking.com/";

        public static string HTTP_API_URL = "https://apis.91asking.com/";
        public static string HTTP_URL = "https://apis.91asking.com/otoapi/";
        public static string HTTP_SUBJECT_HTML_URL = "https://apis.91asking.com/commonapi/subject/oto/html?";
        ...
        {
            return Send(HTTP_URL+url, "POST", data, null);
        }

        public static string SendFullUrlGet(string url)
        {
        ...
        {
            return Send(HTTP_URL+url, "GET", null, null);
        }

        public static string SendDelete(string url)
        {
            return Send(HTTP_URL + url, "DELETE", null, null);
        }
        ...
        public static string Send(string url, string method, string data, HttpConfig config)
        {
            ...
            try
            {
                log.InfoFormat("{0}",url);
                if (!AskUtil.IsInternetAvailable())
                {
                    throw (new Exception("无法连接服务器，检查网络连接是否正常"));  
                }

                if (config == null) config = new HttpConfig();
                HttpWebResponse response = GetResponse(url, method, data, config);
                if(response == null)
                {
            ...
            catch (Exception e)
            {
                ...
                result = JsonConvert.SerializeObject(new ResponseModel(1, "网络访问异常"));
            }
            log.Info(result);
            return result;
        }
    ...
    public class HttpConfig
    {
        ...
        {
            this.Timeout = 10000;
            this.ContentType = "application/x-www-form-urlencoded; charset=" + Encoding.UTF8.WebName;
            //this.UserAgent = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/33.0.1750.117 Safari/537.36";
            //this.Accept = "text/html,application/xhtml+xml,application/xml;q=0.9,Resources/Image/webp,*/*;q=0.8";
